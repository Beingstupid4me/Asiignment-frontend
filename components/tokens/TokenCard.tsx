...
export const TokenCard = memo(function TokenCard({
  ...
  return (
    <div className="group relative bg-[#0f0f0f] hover:bg-[#141414] border border-gray-800/40 hover:border-gray-700/60 rounded-lg transition-all duration-200">
      <div className="flex items-start gap-0">
        ...
        <div className="relative flex-shrink-0 py-3">
          <div className="relative w-20 h-20">
            <Avatar className="w-full h-full rounded-lg border-2 border-gray-800/50 group-hover:border-gray-700">
            ...
            <div className="absolute inset-0 flex items-start justify-center pt-1.5">
              <span className="text-white text-[10px] font-bold drop-shadow-lg px-1.5 py-0.5 bg-black/40 rounded">
                {token.name.length > 10 ? token.name.slice(0, 10) + '...' : token.name}
              </span>
            </div>
            {/* Verified/Featured badge */}
            {tokenPair.featured && (
              <div className="absolute bottom-0 right-0 w-4 h-4 bg-green-500 rounded-full flex items-center justify-center ring-2 ring-[#0f0f0f]">
          ...
          <div className="mt-1 text-center">
            <span className="text-[10px] text-gray-600">{token.contractAddress}</span>
          </div>
        </div>

        {/* Main Content */}
        <div className="flex-1 min-w-0 py-3 pr-3 pl-2">
          {/* Top Row - Token Info & Quick Links */}
          <div className="flex items-start justify-between gap-2 mb-2">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <Badge variant="outline" className="text-[10px] px-1 py-0 border-gray-700">
                  📋
                </Badge>
              </div>
                ...
                {percentageChanges.slice(0, 5).map((change, index) => {
                  ...
                  return (
                    <Badge
                      ...
                      className="text-[10px] px-1.5 py-0.5 font-medium flex items-center gap-1"
                    >
                      <span>{icons[index % icons.length]}</span>
                      <span>{Math.abs(change.value).toFixed(0)}%</span>
                      {change.timeframe && <span className="opacity-70">{change.timeframe}</span>}
                    </Badge>
                  );
                })}
              </div>
        ...
        <div className="flex flex-col items-end gap-1 py-3 pr-3 pl-2 border-l border-gray-800/40">
          <div className="text-right mb-1">
            <div className="text-[10px] text-gray-500 uppercase mb-0.5">MC</div>
          ...
          <div className="text-right mb-1">
            <div className="text-[10px] text-gray-500 uppercase mb-0.5">V</div>
